...
//	private final String lookup2cgLoc = "/usr/bin/perl " + "./rus_resources/lookup2cg"; // replaced by cg-conv
	...
	private final String jarOptLookupLoc = "/home/ruskonteaksta/EduardVIEW/rus_resources/hfst-ol.jar";
	private final String optHfstLoc = "/home/ruskonteaksta/EduardVIEW/rus_resources/analyser-gt-desc.ohfst";
...
//		}
	...
	private String runFST_CG(String input) throws IOException,InterruptedException {
        ...
        String inputfileLoc = "/home/ruskonteaksta/EduardVIEW/rus_output/cg3AnalyserInputFiles/cg3AnalyserInput"+timestamp+".tmp";
        String outputfileLoc = "/home/ruskonteaksta/EduardVIEW/rus_output/cg3AnalyserOutputFiles/cg3AnalyserOutput"+timestamp+".tmp";
	log.info("vislcg3 input file name: "+inputfileLoc);
...
//        		" > " + outputfileLoc};
        ...
        String[] textAnalysisPipeline = {
        		...
        		//"/bin/cat " + inputfileLoc + 
        		//" | " + preprocessLoc + // disabled because it's causing miss alignment (removes original tokens)
        		//"/usr/bin/tee /tmp/VIEWdebug1 <" + inputfileLoc + " | " + 
			loadJar + " " + jarOptLookupLoc + " " + optHfstLoc + " <" + inputfileLoc +
			" 2>/tmp/VIEWerr1-java | " + 
			"/usr/bin/tail -n+5" +
			" 2>/tmp/VIEWerr2-tail | " +                       // get rid of the header that hfst-ol.jar produces
			//"/usr/bin/tee /tmp/VIEWdebug2 | " +
			"/usr/bin/cut -f 1-2" +
			" 2>/tmp/VIEWerr3-cut | " +                      // get rid of the weights
			//" | /usr/bin/tee /tmp/VIEWdebug2 | /usr/bin/awk -F '\\t' 'NR>5 { print $1 FS $2 }'" + // use awk instead of tail | cut ??
        		//" /usr/bin/tee /tmp/VIEWdebug3 | " + 
			cgConvLoc +
			" 2>/tmp/VIEWerr4-conv | " + 
			//"/usr/bin/tee /tmp/VIEWdebug4 | " + 
			vislcg3Loc + " -g " + vislcg3DisGrammarLoc +
			" 2>/tmp/VIEWerr5-cg3 " +  // disambiguate with the constraint grammar
        		//"| /usr/bin/tee /tmp/VIEWdebug5" + 
        		" >" + outputfileLoc };
		...
		log.debug("Text analysis pipeline exit value: " + processExitValue);
		...
		//log.warn("Temporary files NOT deleted!!!");
		inputfile.delete();
		outputfile.delete();
        
//        // always keep the latest two input/output files, delete the oldest files 
//        
//        File inputFileDir = new File("/home/ruskonteaksta/EduardVIEW/rus_output/cg3AnalyserInputFiles/");
...
//		}
//		
//		File outputFileDir = new File("/home/ruskonteaksta/EduardVIEW/rus_output/cg3AnalyserOutputFiles/");
